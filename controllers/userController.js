...
// 삭제 로직은 services/userPurge 한 곳에만 둔다 — 관리자 삭제(adminUserController)와 공유한다.
...
// DELETE /api/user
// **하드 삭제** — 계정과 딸린 데이터를 영구 삭제한다. 실제 삭제는 services/userPurge 가 한다.
//
...
//
// ⚠️ 삭제 범위·순서를 바꿀 일이 있으면 services/userPurge.js 를 고칠 것.
//    관리자 삭제(DELETE /api/admin/users/:uid)가 같은 함수를 쓴다.
const deleteAccount = async (req, res, next) => {
  try {
    const result = await purgeUser(req.user.uid);
    if (!result) {
      return res.status(404).json({ success: false, error: '유저를 찾을 수 없습니다' });
    }
